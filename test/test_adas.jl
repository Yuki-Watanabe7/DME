@testset "ADASModel" begin
    ...
    m = ADASModel(100.0, 0.8, 200.0, 50.0, 100.0, 100.0, 0.2, 100.0, 300.0, 1500.0, 500.0, 1.0)
    ...
    @testset "需要ショック（M増加→Y上昇・P上昇）" begin
        # M を300から400に増加（金融緩和）
        m_monetary = ADASModel(100.0, 0.8, 200.0, 50.0, 100.0, 100.0, 0.2, 100.0, 400.0, 1500.0, 500.0, 1.0)
        ss_base      = steady_state(m)
    ...
    @testset "供給ショック（P_e上昇→Y低下・P上昇）" begin
        ...
        m_pe_shock = ADASModel(100.0, 0.8, 200.0, 50.0, 100.0, 100.0, 0.2, 100.0, 300.0, 1500.0, 500.0, 1.1)
        ss_base     = steady_state(m)
    ...
    @testset "adas_shock_compare" begin
        m_demand = ADASModel(100.0, 0.8, 200.0, 50.0, 150.0, 100.0, 0.2, 100.0, 300.0, 1500.0, 500.0, 1.0)
        result = DME.adas_shock_compare(m, m_demand; scenario_names = ("baseline", "demand_expansion"))
