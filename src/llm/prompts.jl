...
function build_data_comparison_prompt(ctx::AnalysisContext)::String
    ...
    stats_str = isempty(stats_lines) ?
        "  （乖離統計情報なし）" :
        join(["  - " * l for l in stats_lines], "\n")

    data_caveat_items = vcat(dcs.data_caveats, ctx.caveats.data_limitations)
    model_caveat_items = vcat(ctx.caveats.model_limitations, ctx.caveats.interpretation_warnings)
    all_caveat_items = vcat(data_caveat_items, model_caveat_items)
    caveats_str = isempty(all_caveat_items) ?
        "  （特になし）" :
...
function explain_data_comparison(ctx::AnalysisContext)::ExplainDataComparisonOutput
    ...
    # 何を比較したか
    what_compared = "$(meta.model_name) のシミュレーション結果（シナリオ: $(srs.scenario_name)）" *
        " と 実データ（出典: $(dcs.data_source)）の比較です" *
        "（比較期間: 期$(dcs.comparison_period[1])〜$(dcs.comparison_period[2])、" *
        "シミュレーション期間: $(srs.n_periods) 期）。"

    # 乖離の大きい変数
    large_dev, well, poorly = _classify_deviation_variables(dcs.deviation_statistics, meta.model_name)
...
function _classify_deviation_variables(
    ...
    # 相関係数またはRMSEで変数を分類する
    corr_by_var = _extract_per_variable_metric(stats, ["correlation_by_variable", "corr_by_variable"])
    rmse_by_var = _extract_per_variable_metric(stats, ["rmse_by_variable", "RMSE_by_variable"])
    ...
    if !isempty(corr_by_var)
        ...
        well = if isempty(high_corr)
            ...
            high_names = join(["$(k)（相関: $(round(v; digits=3))）" for (k, v) in high_corr], "、")
        ...
        poorly = if isempty(low_corr)
            ...
            low_names = join(["$(k)（相関: $(round(v; digits=3))）" for (k, v) in low_corr], "、")
    ...
    if !isempty(rmse_by_var)
        ...
        large_names = join(["$(k)（RMSE: $(round(v; digits=4))）" for (k, v) in large_vars], "、")
        small_names = join(["$(k)（RMSE: $(round(v; digits=4))）" for (k, v) in small_vars], "、")

        large_dev = "RMSEが大きい変数（参考）: $(large_names)。乖離の原因は断定できません。"
        well = "このモデルの仮定のもとでは、$(small_names) でRMSEが比較的小さく観察されました（参考）。"
        poorly = "このモデルの仮定のもとでは、$(large_names) でRMSEが大きく観察されました（参考）。" *
    ...
    # 統計情報が十分でない場合のデフォルト
    generic = "比較指標（RMSE・MAE・相関係数等）の変数別内訳が提供されていないため、" *
...
function _extract_per_variable_metric(stats::Dict{String, Any}, keys::Vector{String})::Dict{String, Float64}
    for k in keys
...
function _suggest_additional_series(model_name::String, dcs::DataComparisonSummary)::Vector{String}
    suggestions = [
