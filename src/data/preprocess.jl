...
function fill_missing(s::DataSeries; method=:forward)
    ...
    return DataSeries(s.id, s.name, s.source, s.frequency, s.unit, copy(s.dates), vals, meta)
end
...
function drop_missing(s::DataSeries)
    ...
    return DataSeries(s.id, s.name, s.source, s.frequency, s.unit, new_dates, new_values, meta)
end
...
function apply_log(s::DataSeries)
    ...
    return DataSeries(s.id, s.name, s.source, s.frequency, new_unit, copy(s.dates), new_vals, meta)
end
...
function difference(s::DataSeries; periods::Int=1)
    ...
    return DataSeries(s.id, s.name, s.source, s.frequency, new_unit, new_dates, new_vals, meta)
end
...
function moving_average(s::DataSeries; window::Int=3)
    ...
    return DataSeries(s.id, s.name, s.source, s.frequency, s.unit, copy(s.dates), new_vals, meta)
end
...
function standardize(s::DataSeries)
    ...
    new_vals = Union{Float64,Missing}[ismissing(v) ? missing : (v - mu) / sigma for v in s.values]

    meta = deepcopy(s.metadata)
    _append_transform!(meta, "standardize(mean=$mu, std=$sigma)")
    return DataSeries(s.id, s.name, s.source, s.frequency, "standardized", copy(s.dates), new_vals, meta)
end
...
function to_quarterly(s::DataSeries; method::Symbol=:mean)
    ...
    for (date, val) in zip(s.dates, s.values)
        ...
        m === nothing &&
            throw(ArgumentError("月次日付形式が不正です: '$(date)'. YYYY-MM 形式が必要です"))
        year = m.captures[1]
    ...
    return DataSeries(s.id, s.name, s.source, Quarterly, s.unit, new_dates, new_values, meta)
end
...
function to_annual(s::DataSeries; method::Symbol=:mean)
    ...
    for (date, val) in zip(s.dates, s.values)
        ...
        m === nothing &&
            throw(ArgumentError("四半期日付形式が不正です: '$(date)'. YYYY-Qn 形式が必要です"))
        year = m.captures[1]
