...
| [マクロイベント変換契約](docs/architecture/macro_event_contract.md) | 観測イベント/解釈シグナル/シナリオ仮定/適用モデル入力の4層分離・共通イベント属性・イベント型10種のモデル入力マッピング表・適用先を外生変数7個に限定する決定・magnitude捏造禁止・同時/競合/重複イベントの決定論的処理・イベントログと再現契約・API境界6責務・改訂節（`:other` の層限定・シナリオ仮定の時点指定2基準・部門集約の非実装・長期金利/funding-cost shockの独立イベント型追加とpass-through明示化） |
...
| [Japan Fiscal Scenario Lab capability / mapping 契約](docs/architecture/japan_fiscal_scenario_capability.md) | 5 scenario family（低成長+高金利・財政再建・金融抑圧・高成長/生産性・JGB funding-cost）× 候補モデル11種=55セルのrepresentability判定（`representable`/`partial`/`not_representable`）と判定規則・明示的Scenario Assumption概念9種の単位/時点基準とイベント層target conceptとの対応・family別の分解規則と禁止代理の列挙・モデル別mapping（変数/単位/時間軸/換算）と#276への実装要件・gap register 15件（`G-01`利付き政府債務ストック不在・`G-02`日本較正済みモデル不在・`G-03`GDP成長率パス入力不在ほか）・FRE contextを`observed_context_only`に固定しaffinity/share/confidenceをmagnitudeへ変換しない契約・`claim_level`による「言えること/言えないこと」の分離（Issue #274） |
| [Japan Fiscal Scenario Lab claim-level / coverage 契約](docs/architecture/japan_fiscal_claim_level_contract.md) | #274 の capability findings を #275 scenario schema・#276 result artifact・#277 fixture・Market Analyzer consumer へ lossless に伝播する契約・`claim_level` 4段階ごとの主張可能な診断と `numeric_semantics`（model-unit相対/正規化偏差/日本の量）・現在55セルに `magnitude` が0件であることのload時invariant・representability と coverage（概念/出力/チャネル）の分離・family と同粒度の因果チャネル25本と被覆状況（F1成長・F3 BOJ/JGB吸収・F5 sovereign legがunsupported）・`family_complete` が全セルfalseであること・禁止する主張8種・実行時検証 `japan_fiscal_validate_claims` と違反コード7種・downstream handoff requirements 22件（`H-01`–`H-22`）・日本較正で `claim_level` を上げる際のversioning rule（Issue #285） |
| [Japan Fiscal Scenario Lab scenario catalog / explicit assumption schema / FRE context 契約](docs/architecture/japan_fiscal_scenario_schema_contract.md) | FRE snapshot を保持する`JapanFiscalFREContext`（observed_context_only）と明示的assumptionを保持する`JapanFiscalScenarioAssumption`の型分離・5 scenario familyの catalog を `japan_fiscal_family_spec`（#274）からそのまま導出し独自に再定義しない決定（`H-01`）・magnitudeの0とmissingを「concept ごとに assumption が存在するか」で区別する設計・`magnitude_source=:external_belief`のconstruction時拒否（`H-04`）・`unit`/`direction`をconcept/magnitudeから導出しフィールドとして二重保持しない設計・`japan_fiscal_assumption_set_hash`（assumption集合のみ）と`japan_fiscal_scenario_content_hash`（scenario全体、両contract versionを含む）の2種のhash分離によるFRE context非依存性の保証（`H-05`）・hash自己参照排除・RFC 8785正準化+SHA-256・fail closed decode（未知キー/hash改変の検出）・`japan_fiscal_scenario_schema_contract()`によるMarket Analyzer向け単一Dict export（Issue #275） |
| [Japan Fiscal Scenario Lab model adapter / scenario runner / result artifact 契約](docs/architecture/japan_fiscal_scenario_result_contract.md) | #274が採用した14セル（`:primary` 5・`:supporting` 9）を一般macro-eventレイヤー（`Scenario`/`run_scenario`/`map_event`）を経由せず各モデルの既存public API（`capex_run`/`_sim_run`/`impulse_response`/`transition_path`/`*_shock`/`*_comparison`比較ヘルパ）で直接実行する決定・`JapanFiscalScenarioAssumption`がtiming/persistenceを持たないため「評価区間の先頭期からの恒久step」を唯一の適用規則に固定する決定・9モデル×14セルのconcept→variable単位換算表・`claim_level`が許す診断だけを計算し`:direction_only`では`peak`/`onset`等をフィールド自体作らない設計・`japan_fiscal_coverage`を`to_dict`のまま丸ごと埋め込みH-06を満たす設計・F5の`funding_cost_legs`でsovereign leg（常に`unsupported`）とprivate pass-through legを分離（H-11）・`japan_fiscal_validate_claims`を実行前に呼び違反時はartifactを生成しない（H-07）・baseline/scenarioを常に同一baselineパラメータから adapter 内部で導出し一致をランタイム検証でなく構成上保証する設計・`Union{JapanFiscalScenarioResult,JapanFiscalScenarioRejection}`型分岐によるnot_adopted 41セルの機械可読な拒否・Greek文字パラメータ名を値配列としてハッシュしASCII-only制約を満たす設計・`generated_at`除外+自己参照排除のhash・atomic write（ADR 0008と同型）・`japan_fiscal_result_artifact_contract()`によるMarket Analyzer向け単一Dict export（Issue #276） |
| [部門別CAPEX・信用循環モデル 実証統合設計](docs/architecture/capex_credit_cycle_empirical_integration.md) | 実証化契約と実装済みモデル層/イベント層/データ層の整合レビュー結果（差異30件 `Z-01`–`Z-30` と解決先の割当・#170への改訂反映・限界として保持する8件）・catalog→raw observation→measurement→dataset→較正/識別/推定→履歴再生→検証/robustness の7段一方向データフローと段別の禁止事項・`src/data/` 4本と `src/analysis/` 7本の配置とinclude順序/export・公開型23個/公開関数23個/公開定数21個のシグネチャ契約・失敗契約の3層分離（例外/構造化拒否/警告）と段別ステータス語彙6種・標本端点を `:calibration_required` の inner join のみで決める規則と `BOP` の扱い・`vintage_mode` を `:latest_only` の1値に固定する決定・パラメータ6区分のコード上の注入点と `structural` 引数の新設・逆較正48 targetキーの観測対応表と `source_kind` 4値・`ext_demand_s^{ss}` の分割が識別されないことの明示と識別仮定・`EST` 総数35への修正・履歴再生を `compose_exogenous_paths`+`capex_run` の3段で構成しイベント層APIを変更しない決定・助走区間を定常固定にする決定・検証7 dimensionと総合スコア/passed/一致率フィールドを型に作らない担保・robustness 7軸の1軸ずつ実行・hash 5種とprovenance連結・テスト戦略62項目・実装作業の分解 `P-1`–`P-11`（#241–#251） |
...
| [ADR 0020: Japan fiscal scenario capability契約](docs/adr/0020-japan-fiscal-scenario-capability-contract.md) | 5 scenario family×11モデル=55セルの判定を実装前に固定し宣言的レジストリで持つ・representabilityを宣言値でなくfamily仕様（required_concepts/required_outputs）からの導出値として強制する・表現不能な概念を`:not_accepted`行として理由つきで記録し禁止代理をfamily別に列挙する・FRE snapshotを`observed_context_only`に固定し`:external_belief`を受理しない・金融抑圧を単一政策金利ショックへ縮約せず3概念へ分けAD-ASを`not_representable`と判定する・`claim_level=:magnitude`を`japan_calibrated`のときのみ許し該当セルを0にする・GDP成長率パスを受け取るモデルが存在しないことを確定し生産性ショックと区別する・VARを構造識別不在により全familyで`not_representable`とする・JGB funding-costをsovereign legとprivate pass-through legに分け#260契約の再利用範囲を要素別に確定する・ギャップ15件を限界として登録する・familyごとに`:primary`を1つだけ置きモデル合成を行わない・モデル方程式と既存APIを変更しない決定記録 |
| [ADR 0021: Japan fiscal claim-level契約](docs/adr/0021-japan-fiscal-claim-level-contract.md) | `claim_level` を「主張してよい診断」と「数値系列の意味づけ」へ機械可読に展開する・`magnitude` 0件をload時invariantとして検査する・representability と coverage を別フィールドで追跡し #274 registry から導出する・family と同粒度の因果チャネルを宣言し `family_complete` を全セルfalseとする・未較正モデルの数値系列は保持しつつラベルを `numeric_semantics` で制御する・契約を文書でなく実行時検証関数として提供し未対応の非開示も違反とする・禁止する主張8種を語彙として固定する・downstream handoff requirements 22件を宣言的レジストリで持つ・`claim_level` 昇格の条件とversion規則を先に固定しconsumerの暗黙昇格を禁じる・#274 registryとモデル方程式を変更しない決定記録 |
| [ADR 0022: Japan fiscal scenario schema契約](docs/adr/0022-japan-fiscal-scenario-schema-contract.md) | FRE contextを`JapanFiscalFREContext`として型分離しmagnitudeを計算する関数の引数に一切現れないようにする・0とmissingの区別をnullable fieldでなく「assumptionが存在するか」で表す・`unit`/`direction`をconcept/magnitudeからの導出値としフィールドとして二重保持しない・`magnitude_source=:external_belief`をconstruction時に拒否する・scenario catalogを#274の`japan_fiscal_family_spec`からそのまま導出し独自に再定義しない（load時invariantで検査）・family単位のhorizon/persistence代表値をcatalogに持たせずper-model粒度の誤読を避ける・content_hashをprovenanceのフィールドとして持たずhash自己参照を排除する（ADR 0008と同型）・`assumption_set_hash`（context非依存）と`scenario_content_hash`（両contract version含む）を分離しH-05をテストでなく関数の性質として満たす・fail closed decodeで未知キー混入とhash改変を検出する決定記録 |
| [ADR 0023: Japan fiscal scenario result artifact契約](docs/adr/0023-japan-fiscal-scenario-result-artifact-contract.md) | `JapanFiscalScenarioAssumption`がtiming/persistenceを持たないため評価区間の先頭期からの恒久stepを唯一の適用規則に固定する・9モデル（CCCを含む）いずれも一般macro-eventレイヤー（`Scenario`/`run_scenario`/`map_event`）を経由せず各モデルの既存public APIを直接呼ぶ・診断を`claim_level`が許す範囲だけ計算しnullで隠すのではなくフィールド自体を作らない・`result_shape`を`:time_path`/`:static_point`の2値に限定しmodel別のcase分けを1ファイルへ閉じ込める・`japan_fiscal_run`の戻り値を`Union{Result,Rejection}`にしstatus flagを1つのstructへ持たせない・familyを引数に取らずscenario.familyから読みbaseline/scenarioを同一baseline paramsから内部導出することで一致をランタイム検証でなく構成上保証する・`japan_fiscal_coverage`をto_dictのまま丸ごと埋め込む（H-06）・F5のsovereign/private leg分離（H-11）・Greek文字パラメータ名を値配列としてハッシュしASCII-only制約を満たす・hash自己参照排除+atomic write（ADR 0008と同型）・baselineは各モデルの既存illustrativeパラメータのみを用い新しい日本較正を発明しない決定記録 |
| [ADR 0019: 長期金利・funding-cost shockの分離契約](docs/adr/0019-long-rate-funding-shock-contract.md) | イベント型を9種から10種へ拡張し `:LongRateFundingShock`（長期金利・funding条件ショック）を追加する・新しい exogenous 変数を追加せず既存の `spread_shock_ex` を適用先とし `:CreditSpreadShock`/`:RefinancingOrRatingEvent` と同じ加算スロットへ合成する・生データ分解（`FundingShockComponents`）とモデル入力変換（`FundingShockPassThrough`、versioned）を分離し暗黙の1:1マッピングにしない・`long_real_yield_shift_bps`/`inflation_compensation_shift_bps`から導出する残差を`term premium`と呼ばない・`credit_spread_shift_bps`は既存`:CreditSpreadShock`に委ね二重計上を避ける・`allowed_target_concepts`により`:PolicyRateChange`との型レベルの独立性を保証する決定記録（Issue #260 Part A） |
| [ADR 0018: 部門別CAPEX・信用循環モデルの実証実装契約](docs/adr/0018-capex-credit-cycle-empirical-runtime-contract.md) | 実証層を7段の一方向データフローとし後段が前段を書き換えない・系列catalogの正本をDME内のversioned Julia constとし `:firm_disclosure` が較正roleを持つことをvalidatorで拒否する・providerが返さないmetadataを `missing` として保持しcatalogの宣言値で埋めない・`capex_credit_cycle_model` に非破壊の `structural` を追加し `CAL-SS` の上書きを例外にする・逆較正48キーに `source_kind` を付け `:observed` でないキーを観測較正値として提示しない・`ext_demand_s^{ss}` と `order_gen_s^{ss}` の分割が識別されないことを明記し識別仮定を固定する・方程式別残差をモデル層に公開APIを足さず推定層に閉じて実装し一致をテストで統制する・弱識別対応を事前適用と事前固定/事後発火の2段に分ける・履歴再生を `run_scenario` ではなく `compose_exogenous_paths`+`capex_run` で構成しイベント層APIを変更しない・助走区間の外生を定常固定にし `runup_tol` を緩めない・履歴episodeで `L1` から `L3` を自動生成しない・`data_vintage` の未申告を `"unknown"` とし `vintage_mode` を1値に固定する・`emp_s4` 不在により `EST` 総数を35へ修正する・失敗を3層に分け段別ステータス語彙を固定する・検証に総合スコア/`passed`/観測側増幅度/ラベル一致率のフィールドを型として作らない・robustnessを1軸ずつ実行しbest-fitを自動採用しない・`SimulationResult` 非変更・artifact identityを5種のhashで連結し `retrieved_at` 等を対象外にする決定記録 |
...
| [2026-09 Financial-Instability Live Holdoutデモ](docs/examples/financial_instability_holdout_demo.md) | Part A（`:LongRateFundingShock`）・Part B（EDP financial-stress 8系列consumer）・Part D（構造化診断）を接続し `FinancialInstabilityAssessment`（trigger/weak_credit/funding/broad_conditions/model_amplification/minsky_diagnosticの6次元 + rule-based overall_status）を構築・保存する再現可能なデモの実行手順・overall_statusのルール（trigger単独では confirmed/watch にならない）・成果物・入力データがfictionalであることの明記（Issue #260 Part D。既定のfixtureモードはAPIキー不要・ネットワークアクセスなし・決定的）。同ファイルは `select_observation_window`（現在の市場水準を見ずに事前固定したラックバックで observation window を機械的に選ぶルール）・`run_financial_instability_holdout_live_snapshot`（`DME_DATA_MODE=live` 必須の pre-FOMC canonical live snapshot エントリポイント）・`run_manifest.json`（DME code revision・rule/threshold/catalog version・EDP identity・observation window・各系列provenanceのfreeze記録）も提供する（Issue #271 Part A。liveモードはAPIキー・ネットワークアクセスが必要）。さらに `run_financial_instability_post_fomc_comparison`（保存済みpre-FOMC artifactを読み post-FOMC snapshotを再実行し比較する Part B/C/D 統合エントリポイント）・`src/analysis/financial_instability_comparison.jl` の `compare_financial_instability_assessments`（dimension別pre/post差分・`hypothesis_strengthened`/`hypothesis_weakened`/`no_material_change`/`insufficient_comparable_data` の4値conclusion）・`build_financial_instability_handoff`（観測事実/DME rule-based interpretation/未検証の経済解釈を区別するclassification付きfinance-checker handoff artifact。belief/Evidence/Hypothesisは自動更新しない）を提供する（Issue #271 Part B・C・D） |
| [部門別CAPEX・信用循環モデル 実証統合デモ](docs/examples/capex_credit_cycle_empirical_demo.md) | 合成（synthetic）データでcatalog→raw observation→measurement/dataset→定常水準較正・識別診断・限定推定（literature_default/calibrated/estimated）→historical episode選定→履歴再生→dimension別validation→robustness/sensitivityの7段を完走し、identity chain付きcanonical artifact・人間可読レポートを保存する再現可能な統合デモの実行手順・成果物・結果の限界（APIキー不要・ネットワークアクセスなし・決定的） |
