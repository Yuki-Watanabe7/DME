...
| [Julia品質Export Contract](docs/contract/julia-quality-export-v1.md) | `software-quality-dashboard` 連携用に DME が所有する versioned contract（`julia-quality-export/v1`。real-rate model artifact とは逆方向）・トップレベル構造・予約ツール名7種と対応Issue・status5値ごとの必須/禁止フィールド表・redaction方針（自由記述は自動redact/構造化データはreject）・versioning方針・Julia API早見表・Exporter骨格の実行方法・限界 |
