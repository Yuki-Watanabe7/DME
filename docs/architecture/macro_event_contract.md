...
| **event contract version** | `macro-event-contract/1.0.2` |
...
| **改訂の優先関係** | **§12（#171 統合レビューによる改訂）と §13（#196 統合設計による改訂）が本書の正本である。§3.3 の単位 × 適用方式の許容表は §12 で、§3.1 の `event_type` と `effective_from`・§5.3 の集約規則は §13 で上書きされている。** |
...
## 13. #196 統合設計による改訂（`1.0.2`）

本節は #196 の統合設計（[イベント・シナリオ実行層 統合設計](macro_event_runtime_integration.md) §2）で検出された本書と実装可能性の差異 3 件（`Y-01`・`Y-02`・`Y-03`）を記録する。**本節は §12 とともに本書の正本であり、本文の該当箇所と矛盾する場合は本節が優先する。**

### 13.1 `event_type` の `:other` を層で限定する（`Y-01`）

**検出された差異**: §3.1 は `event_type` を「§4 の 9 種のいずれか、または `:other`」と定めるが、`:other` を全層で無条件に許すと、適用先も検証規則も持たないイベントが `L3`・`L4` へ到達しうる。実装側の要件（未知・未対応のイベント型を generic event へ落とさず拒否する）と両立しない。

**改訂**: `:other` の許容範囲を層で限定する。

> `event_type` の `:other` は **`L1`（Observed Event）・`L2`（Interpreted Signal）に限り許容する**。`L3`（Scenario Assumption）・`L4`（Applied Model Input）では `:other` を拒否する（`unsupported_event_type`）。§4 の 9 種にも `:other` にも該当しない値は、**全層で拒否する**。

**根拠**: 「観測はしたが解釈できない事象を記録する」ことと「モデルへ適用する」ことは別の要求である。前者を認めないと `L1` が観測の完全な記録にならず、後者を認めると §4.2 のマッピング表を経ないイベントがモデル入力へ到達する。層で分けることで両立する。

### 13.2 `L3` の時点指定を 2 基準とする（`Y-02`）

**検出された差異**: §3.1 は `L3` で `effective_from`（暦日）を必須とするが、[時間軸](scenario_time_semantics.md) §9-4 は「`period_zero` を持たないシナリオでは日付付きイベントを扱えない。理論シナリオと履歴再生で扱いが分かれることを実装が明示する必要がある」と限界を記録している。実装済みの理論シナリオ（`Sc0`–`Sc4`）は `t = 0` 起点の整数時点のみを持ち、暦日を持たない。暦日を必須にしたまま理論シナリオを `L3` として表現するには、観測されていない日付を割り当てるほかなく、§3.2 の捏造禁止と衝突する。

**改訂**: `L3` の時点指定を 2 基準とする。詳細な規則は [時間軸](scenario_time_semantics.md) §11.1 が定める。

> `L3` は次のいずれか **一方のみ** を完備する。
> - **暦日基準**: `effective_from`（必須）+ 適用四半期の割当規則（§4.3・[時間軸](scenario_time_semantics.md) §4.3）
> - **モデル期基準**: 適用四半期 `t_apply` の直接指定（暦を持たない理論シナリオ用）
>
> 1 つのシナリオ内で 2 基準を混在させない（`mixed_timing_basis` として拒否）。モデル期基準の `L3` は、暦日から導出された時点ではないことを警告として必ず記録する。

**根拠**: 属性表 §3.1 の `effective_from` は「経済的効力の開始日を必ず確定させる」という要求であり、その要求は `t_apply` の直接指定でも満たされる。暦日という**表現形式**を必須にしていたことが理論シナリオを排除していた。混在を拒否するのは、§5.1 のソートキー第 4 要素（`effective_from`）が比較不能になるためである。

### 13.3 企業レベルイベントの部門集約を実装しない（`Y-03`）

**検出された差異**: §5.3 は企業レベルイベントを部門レベルへ集約する規則（`entity_weight`・カバレッジ）を定めるが、実装側では集約推定を対象外としている。集約を実装しないまま `entity_weight` 属性だけを型に持たせると、等ウェイト代用（§5.3-1 が禁じる）を書く余地が残る。

**改訂**: §5.3 の集約規則の実装状態を明示する。

> §5.3 の集約規則は **初期実装の対象外**である。`entity` を持つ `L2` から `L3` を自動生成する API を提供せず、分析者が部門レベルの `L3` を与える。`entity` が非空の `L3` は `aggregation_not_implemented` として拒否する。`entity_weight` は**属性として持たない**（存在しない自由度を作らない）。
>
> 部門集約を実装する場合は §5.3 の規則をそのまま用い、`entity_weight` の出所・基準期・カバレッジの出力義務を同時に満たす。カバレッジを 1 に正規化しない・等ウェイトで代用しないという規律（§5.3-1・§5.3-2）は実装時点で有効になる。

**根拠**: 規則を文書に持ちながら実装しない状態は正常である（規則は将来の実装が従うべき制約である）。危険なのは、規則を満たさない実装が「集約」として通ることである。属性そのものを持たせなければ、等ウェイト代用は書けない。

### 13.4 差し戻し事項 `D1`–`D4` の状態

`D1`–`D4`（§8）は本改訂の対象外であり、状態は変わらない。イベント層の実装は、これらに該当するイベント型を `unmapped_target` として拒否し、拒否理由に差し戻し ID を含める（[統合設計](macro_event_runtime_integration.md) §6.2）。
