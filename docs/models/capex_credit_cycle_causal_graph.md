...
> 関連 Issue: #164（本書）・#163（分析契約）・#169（`1.1.0` の改訂要求元）・#125（ロードマップ）
> 前提: [部門別CAPEX・信用循環モデル 分析契約](capex_credit_cycle_analysis_contract.md)（基準ユースケース・判定問題 Q1–Q5・シナリオ `Sc0`–`Sc4`・診断ラベル）
> 後続設計: #165（[部門境界と変数定義](capex_credit_cycle_sectors_variables.md)）・#166（[ストック・フロー会計表](capex_credit_cycle_stock_flow.md)）・#167（[責務境界](capex_credit_cycle_model_boundaries.md)）・#168（[イベント変換契約](../architecture/macro_event_contract.md)・[シナリオ時間軸](../architecture/scenario_time_semantics.md)）・#169（[動学方程式](capex_credit_cycle_equations.md)）・#170（観測・検証）・#171（統合）
...
| **graph version** | `capex-credit-cycle-graph/1.1.0` |
...
6. **`EXT` エッジを初期MVPの実装に含めない**。含める必要が生じた場合は本書を改訂してから行う（§11）。
...
| 記法 | 意味 |
|---|---|
| `s ∈ SP = {S2, S3}` | 半導体設計・製造（`S2`）と製造装置・建設・電力（`S3`）に共通のエッジ。両部門へ同一構造で適用する |
| `s ∈ SF = {S1, S2, S3}` | 財務主体（債務・現金・資本コストを持つ借り手部門）に共通のエッジ。収益ブロック（§3.4）とブロック D（§3.5）の添字集合 |
...
| `Y_S1` | AI・クラウド産出 | 産出増 | `P` | — |

**`1.1.0` での追加**: `Y_S1` は `1.0.0` に存在せず、#165 §7 の差し戻し `A1`（`S1` に産出・売上ノードが無いため `CASH_S1` を生成できず、`L41` が経済的に意味を成さない）として登録されていた。本版で追加し、`A1` を解決する（§10.1）。`PRICE_S1` は初期MVPで外生であるためノードを置かない（#165 §5.2）。

### 2.2 S2 / S3 固有ノードと財務主体共通ノード

生産構造に固有のノードは `s ∈ SP = {S2, S3}`、収益・資金のノードは `s ∈ SF = {S1, S2, S3}` を添字集合とする。
...
**`1.1.0` での変更**: 収益ブロックの添字集合を `SP` から `SF` へ拡大した。`1.0.0` はこれらを「S2 / S3」の節に置きながら、ブロック D の `L39`・`L41` で `s` に `S1` を含めており（§3.5）、添字集合が文書内で一貫していなかった。`Y_S1` の追加（§2.1）により `SALES_S1 ← PROFIT_S1 ← OCF_S1 ← CASH_S1` の系列が閉じ、`L41`（`CASH_S1 → CAPEX_EXEC`）が経済的に成立する。`S1` は在庫・受注残・稼働率・価格を持たないため、生産部門固有ノードには含めない。
...
flowchart LR
  ...
  subgraph SFB["収益・資金（s ∈ SF = S1・S2・S3）"]
    SALES["売上"]
    PROFIT["利益"]
    OCF["営業CF"]
    CASH["現金保有"]
...
> 図では `S2`・`S3` を 1 組のノードとして描いている。実際には `s ∈ SP` の各部門に同一構造が存在し、
> `L09`・`L19`・`L50` の受注エッジは部門別の需要構成比で分配される。収益・資金ノード（`SALES`–`CASH`）は
> `s ∈ SF` の各部門に存在し、`Y_S1 --L20--> SALES` は `s = S1` の、`Y_s --L20--> SALES` は `s ∈ SP` の
> インスタンスである。`EXT` エッジ（§3.8）は図に含めていない。

### 3.2 ブロック A: 期待 → 計画 → 実行 CAPEX → 産出（`S1`）

| ID | 原因 → 結果 | 型 | 符号 | 遅れ | 関数形候補 | 観測 | 実装 |
|---|---|---|---|---|---|---|---|
| `L01` | `AI_EXP` → `COMPUTE_DEM` | 行動 | `+` | 0 | linear | `L`→`P` | MVP |
| `L02` | `COMPUTE_DEM` → `TARGET_CAP` | 行動 | `+` | 0 | linear（資本係数 ÷ 目標稼働率） | `P`→`L` | MVP |
...
| `L60` | `COMPUTE_DEM` → `Y_S1` | 行動 | `+` | 0 | saturating（需要と能力の小さい方） | `P`→`P` | MVP |
| `L61` | `CAP_S1` → `Y_S1` | 制度 | `+` | 0 | saturating（能力上限） | `P`→`P` | MVP |

**根拠・注意**

- `L01`–`L03`: 加速度原理（accelerator）に基づく投資決定。期待需要から導かれる目標資本ストックへ部分調整する標準形。`L03` の調整速度は閉路に属さないが、`R1a`・`R1b`・R2 の**作動の速さ**を規定する。
...
- `L60`・`L61`（`1.1.0` で追加）: `S1` の産出は期待需要と自社の設備能力の**小さい方**で決まる。`L60` が需要側、`L61` が能力上限側であり、`L58`/`L13` と同様に**片方だけを実装してはならない**。初期MVPの基準ユースケース（需要期待の下方修正）では需要側が拘束するため `L61` は非拘束になり、`R1a` の利得はほぼ 0 になる（§4 `R1a`）。この非対称性を「`S1` に内部資金ループが無い」と読み替えない。
- **`CAPEX_EXEC` の認識基準（#166 差し戻し `B3` の確定）**: `CAPEX_EXEC` は**発注・引渡・支払が同一期に生じる支出額**として解釈する（#166 §3.5 の仮定 A-2・§4.4）。したがって `L09`（`CAPEX_EXEC → ORDER_s`）で生まれた資本財需要は `BACKLOG_s` に滞留せず、当期に全額引き渡される。`L11`（`BACKLOG_s → Y_s`）の飽和が作用するのは一般需要・モデル外需要の未充足分のみである。発注から稼働までの遅れは `capex_pipe_s` の滞留（`L08`・`L57` の遅れ `2–4`）として保持される。
...
- **`COMPUTE_DEM` への実績フィードバックを持たない**: `Y_S5`（一般経済産出）や `SALES_S1` から `COMPUTE_DEM` へのエッジは置かない。したがって家計所得の変動は `S1` の需要へ還流せず、`R4`（所得・消費ループ）は `S2`・`S3` 経由に限られる。この非対称性は [動学方程式](capex_credit_cycle_equations.md) §17 の差し戻し `E1` として登録されている。
...
### 3.4 ブロック C: 生産 → 収益 → 金融（`s ∈ SF` → `S4`）

**添字集合**: 本ブロックの `s` は `SF = {S1, S2, S3}` である（§2.2）。`s = S1` のとき `Y_s` は `Y_S1`（§2.1）、`PRICE_s` は外生の `PRICE_S1`（ノードを置かない）に読み替える。

| ID | 原因 → 結果 | 型 | 符号 | 遅れ | 関数形候補 | 観測 | 実装 |
|---|---|---|---|---|---|---|---|
| `L20` | `Y_s` → `SALES_s` | 定義 | `+` | 0 | `PRICE_s × Y_s` | `O`→`O` | MVP |
| `L21` | `PRICE_s` → `SALES_s` | 定義 | `+` | 0 | 同上 | `O`→`O` | MVP |
| `L22` | `SALES_s` → `PROFIT_s` | **会計** | `+` | 0 | 会計残差（固定費レバレッジは費用側の固定性から導出） | `O`→`O` | MVP |
...
- `L22`（`1.1.0` で型を `行動` → `会計` へ変更）: **固定費レバレッジ**により、売上減少は利益をより大きく減少させる。これが `R1b`・R2 の利得を 1 に近づける主要因であり、線形化してはならない。ただし利益は `profit_s = va_s − wagebill_s − dep_s` という**会計残差**であり、売上の関数として独立に生成できない（#166 §4.2・差し戻し `B6`）。非線形性は費用側の固定性（`wagebill_s` の労働退蔵・`dep_s` の完全固定）から**導出される**。型を `会計` としたのはこのためであり、`PROFIT_s` を推定対象の行動方程式として扱わない。`PROFIT_s` の役割は #165 §4.4 で `diagnostic` へ変更されている。
...
| `L41` | `CASH_S1` → `CAPEX_EXEC` | 行動 | `+` | **1** | saturating（手元流動性による信用ショック耐性） | `P`→`O` | MVP |
| `L42` | `LEND_STANCE` → `INVEST_s` | 行動 | `+` | 1 | linear | `O`→`O` | MVP |
| `L62` | `CASH_s` → `INVEST_s`（`s ∈ SP`） | 行動 | `+` | 1 | saturating（内部資金による投資の下支え） | `P`→`O` | MVP |
| `L63` | `ROLLOVER` → `CASH_s`（`s ∈ SF`） | 会計 | `+` | 0 | `repay_s = (1 − ROLLOVER) · 満期到来額` | `P`→`P` | MVP |
| `L64` | `COST_CAPITAL_s` → `INVEST_s`（`s ∈ SP`） | 行動 | `−` | 1 | state-dep（`CASH_s` 依存、`L62` と相互作用） | `L`→`O` | MVP |

**根拠・注意**

- `L39`・`L40`・`L42`・`L64` が **Q2 の増幅度 `A` を生む本体**。`credit-off` 反実仮想では `L39`・`L40`・`L42`・`L64` の弾性を `0` に固定する（`L41`・`L62` は信用チャネルではなく内部資金チャネルなので固定しない）。この対応関係を #169 の実装契約へ渡す。分析契約 §3 Q2 は `credit-off` を「資本コスト・貸出態度・借換条件の各弾性を `0` に固定」と**パラメータ種別で**定義しており、`L64` はその「資本コスト弾性」に含まれる。契約 §3 の改訂を要しない。
- `L41`（`1.1.0` で対象を `CASH_S1` に限定し、遅れを `1` に確定）: **B2（自己資金による信用ショック耐性）**の所在。現金が厚い部門では `COST_CAPITAL_s` 上昇が実行 CAPEX へ伝わりにくい。`L39` の state-dep はこの相互作用を表す。`1.0.0` は結果ノードを `CAPEX_EXEC`（`S1` のノード）としながら原因を `CASH_s`（`s ∈ SP` と読める）としており、「`S2`・`S3` の現金が `S1` の CAPEX を賄う」という経済的に成立しない読みを許していた（#165 §7 の `A2`）。`s ∈ SP` の内部資金経路は `L62` として分離した。遅れ `1` の確定根拠は #166 §2.4・確定記録 `B7`（遅れ `0` は当期 `ocf_s` と当期 `capex_exec_s1` の同時決定を生み、期内処理順序が閉じない）。
- `L62`（`1.1.0` で追加）: `s ∈ SP` における内部資金の投資への作用。`L41` の `SP` 版であり、`R1b`（§4）を閉じるエッジである。`INVEST_s` にも資金制約が働くことは #166 §6.2 の恒等式 2'（`s ∈ SP`）から会計的に必然であり、これを持たないと `SP` の資金不足がどこにも現れない。
- `L63`（`1.1.0` で追加、#166 §6.4 (b)・差し戻し `B5`）: 借換条件の悪化は資本コストの連続的な上昇とは独立に、**借換できなかった元本の現金流出**を生む。`1.0.0` は `ROLLOVER` を `CAPEX_EXEC`（`L40`）へのみ接続しており、この現金制約が経路として存在しなかった。会計エッジであり推定対象ではない（残高更新式は #166 §5.4）。
- `L40`: 借換条件の悪化は資本コストの連続的な上昇ではなく、**投資の延期という離散的な反応**を生む。閾値型にするのはこのため。`L63`（現金流出）と `L40`（延期）は同一の `ROLLOVER` 低下に対する会計的帰結と行動的反応であり、二重計上ではない。
...
- `L44`: データセンター建設・据付は `S3` の雇用を通じて所得へ直接効く。CAPEX 削減が半導体経由よりも早く家計へ届く経路であり、遅れを `0–1` としている。**`EMP` ノードは部門添字を持たないため、`L44` の帰属部門はグラフ上で一意に決まらない**。[動学方程式](capex_credit_cycle_equations.md) §10.1 は `L44` を `S3` の雇用需要のうち建設・据付比率に対応する成分として実装し、`L43`（`Y_s → EMP`）との二重計上を避ける分解を与えている。この帰属の明示は同書 §17 の差し戻し `E2` として登録されている。
...
**表記**: 以下、`R1` は `R1a`（`S1` の内部資金ループ）と `R1b`（`S2`・`S3` の受注・内部資金ループ）の**総称**として用いる。個別に指す場合は `R1a` / `R1b` と明記する。`R1` を単一のループとして利得評価しない（§4.1 の合成利得の契約と同じ理由）。

### R1a: `S1` の内部資金ループ

```
CAPEX_EXEC --L08--> CAP_S1 --L61--> Y_S1 --L20--> SALES_S1
  --L22--> PROFIT_S1 --L23--> OCF_S1 --L24--> CASH_S1 --L41--> CAPEX_EXEC
```

| 項目 | 内容 |
|---|---|
| 経路の意味 | CAPEX 削減 → 設備能力の伸び鈍化 → 産出制約 → 売上・利益悪化 → 内部資金減 → 追加 CAPEX 削減 |
| 符号 | 全エッジ `+` → 1 周 `+`（正のフィードバック） |
| 利得を規定する主因 | `L61`（能力が拘束しているか）・`L22`（固定費レバレッジ）・`L41`（現金の投資への感応度） |
| 非線形性 | `L61` の saturating が支配的。**能力が非拘束（需要 < 能力）の領域では利得はほぼ 0** |
| 遅れ | 1 周で最短 `L08` の 2 + `L41` の 1 = 3 四半期。R1b・R2 より遅い |
| 遮断 | B2（現金）・B4（設備転用による目標能力の下支え） |
| 対応する判定問題 | Q1（産業内収束の一部）・Q5（能力拘束の有無が分岐条件） |
| 信用チャネル | **含まない**。Q2 の `credit-off` 反実仮想でも生き残るループ |

**注記**: 基準ユースケース（AI 需要期待の下方修正）では `COMPUTE_DEM` が下がるため `L61` は非拘束となり、`R1a` の利得はほぼ 0 になる。`R1a` が作動するのは**能力が拘束している状態から CAPEX を削る**シナリオ（例: 供給制約下での計画縮小）である。「利得がほぼ 0 だから登録不要」ではない — 状態依存の利得を持つループを無名にしないことが §1.2-4 の要求である。

### R1b: `S2`・`S3` の受注・内部資金ループ

```
ORDER_s --L10--> BACKLOG_s --L11--> Y_s --L20--> SALES_s --L22--> PROFIT_s
  --L23--> OCF_s --L24--> CASH_s --L62--> INVEST_s --L19--> ORDER_{s'} --L10--> …
```

| 項目 | 内容 |
|---|---|
| 経路の意味 | 受注減 → 産出減 → 利益悪化 → 内部資金減 → 部門自身の設備投資削減 → 相手部門の受注減 |
| 符号 | 全エッジ `+` → 1 周 `+`（正のフィードバック） |
| 利得を規定する主因 | `L22`（固定費レバレッジ）・`L62`（現金の投資への感応度）・`L11`（受注残の吸収度）・`L19`（部門間投資連関の係数） |
| 非線形性 | `L22` の費用固定性と `L11` の saturating。利得は状態依存であり定数ではない |
| 閉路の構造 | 初期MVPでは `X06`（`INVEST_S3 → ORDER_S2`）が `EXT` のため、`S2 → S3` の一方向のみ。したがって閉路は `S3` 内部（`INVEST_S3` の相手が `SX`）では閉じず、**`S2` 起点で `S3` を経由して戻る閉路は存在しない**。実質的に閉じるのは各部門内の `CASH_s → INVEST_s → ORDER_s`（`s = s'` の自己連関を持たないため、初期MVPでは `S2 → S3` の一方向の伝播のみ）である |
| 遮断 | B1（受注残）・B2（現金）・B6（価格→数量） |
| 対応する判定問題 | Q1（このループのみで収束するか） |
| 信用チャネル | **含まない**。Q2 の `credit-off` 反実仮想でも生き残るループ |
| 起点からの伝播 | `CAPEX_EXEC --L09--> ORDER_s` は本ループの**入口**であり、閉路の一部ではない（`L09` の逆向きの経路が存在しない） |

**`1.1.0` での `R1` の再定義（#165 §7 の差し戻し `A2` の解決）**

`1.0.0` の `R1` は次のように定義されていた。
...
`CAPEX_EXEC` は `S1` のノード、`ORDER_s`–`CASH_s` は `s ∈ SP` のノードであるため、最後の `L41` は「`S2`・`S3` の現金が `S1` の実行 CAPEX を増やす」と読め、経済的に成立しなかった。本版では次の 2 点で解決する。

| 措置 | 内容 |
|---|---|
| 閉路の分割 | `S1` 内部で閉じる `R1a` と、`SP` の内部資金・部門間投資で閉じる `R1b` の 2 本へ分割する。両者は `CAPEX_EXEC --L09--> ORDER_s` で**接続されるが同一の閉路ではない** |
| エッジの整理 | `L41` の原因を `CASH_S1` に限定し、`s ∈ SP` の内部資金経路を `L62`（`CASH_s → INVEST_s`）として新設する（§3.5） |

`A2` が挙げた 2 つの意図のうち、**(i) と (ii) を両方とも別ループとして採る**。(i) のみでは `SP` の内部資金制約が経路として存在せず、(ii) のみでは `S1` の内部資金経路（`L41`）が閉路に属さない無名の循環になる。`B2`（自己資金による耐性）の作用先は `R1a`・`R1b`・R2・R3 の 4 本へ更新した（§5）。
...
| 組み合わせ | 相互作用 |
|---|---|
| R1b × R2 | R1b の利益悪化が R2 のカバレッジ低下を生む。R2 の投資削減が R1b の受注減を深める。**両者は `PROFIT_s`・`ORDER_s` を共有し、合成利得は各利得の和より大きくなりうる** |
| R1a × R1b | `CAPEX_EXEC --L09--> ORDER_s` で接続されるが、閉路を共有しない。`R1a` の利得は `L61` が拘束している状態でのみ正になるため、両者が同時に高利得となるのは能力拘束下の CAPEX 削減シナリオに限られる |
| R1a × R2 | `CASH_S1` を経由する `R1a` と `COST_CAPITAL_S1 --L39--> CAPEX_PLAN` を経由する R2 は、`L39` の state-dep（`CASH_S1` 依存）で相互作用する。現金が薄いほど R2 の利得が上がり、同時に `R1a` の内部資金経路も細る |
...
| `B2` | 自己資金・現金保有による信用ショック耐性 | R1a・R1b・R2・R3 | `L41`・`L62`（saturating）と `CASH_s` の初期水準、`L39`・`L64` の state-dep | MVP | Q2・Q5 |
...
|---|---|---|
| `contained_adjustment` | `R1a` / `R1b` のみ（かつ B1・B3・B6 が吸収） | 産業内で収束。`S4`・`S5` へ波及していない |
| `sectoral_downturn` | `R1a` / `R1b`（+ R2 または R3） | 信用経路まで作動するが R4 が作動していない |
| `broad_downturn` | `R1a` / `R1b` + （R2 または R3）+ **R4** | 家計経路が作動し、広がり・深さ・持続性の 3 条件を満たす |
...
| 診断量 | 定義の形 | 用途 |
|---|---|---|
| ループ作動フラグ（`R1a`・`R1b`・R2–R4） | 各ループの構成エッジの寄与が閾値を超えたか | §7.2 の整合確認 |
...
| #169 動学方程式 | §3 の型 `行動` エッジの関数形候補と遅れ。`R1a`・`R1b`・R2–R4 の利得評価方式。§7.3 の診断量。`credit-off` で固定するエッジ（`L39`・`L40`・`L42`・`L64`）。`1.1.0` で解決した `A1`・`A2`・`B5`・`B6`（§10.1） |
...
7. **エッジの遅れは候補であり確定値ではない**。`0`・`1`・`2–4` は四半期モデルとしての粒度であり、実際の調整はより連続的でありうる。ただし `L41`・`L62` の遅れ `1` は期内処理順序を閉じるための**確定値**であり、候補ではない（§10.1 の `B7`）。
8. **`R1a` の利得は基準ユースケースでほぼ 0 である**（§4 `R1a` の注記）。`L61`（能力上限）が非拘束であるため、`S1` の内部資金ループは需要主導の下方ショックでは作動しない。`S1` の CAPEX 削減が `S1` 自身の収益を通じて自己増幅する経路は、初期MVPでは能力拘束下に限られる。
9. **家計所得から `S1` 需要への還流経路が無い**（§3.2 の根拠・注意）。`COMPUTE_DEM` は `AI_EXP` のみで駆動されるため、R4（所得・消費ループ）は `S2`・`S3` 経由に限られ、`S1` の産出は家計消費の悪化に反応しない。`broad_downturn` の `breadth`（実体部門 4 部門中の割合）において `S1` が悪化しにくい方向のバイアスが生じる。

---

## 10. 差し戻し事項の解決記録（`1.1.0`）

### 10.1 解決した項目

`1.0.0` に対して #165・#166 が登録した差し戻し事項のうち、本版で解決したものを記録する。解決は #169（[動学方程式](capex_credit_cycle_equations.md)）の設計要件として要求されたものであり、#165 §7 の契約（`A1`・`A2` は #164 の改訂として処理する）・#166 §10.2 の契約（`B5` は #164 の改訂を要する）に従う。

| ID | 登録元 | 内容 | 本版での解決 |
|---|---|---|---|
| `A1` | #165 §7 | `S1` に産出・売上ノードが無く、`CASH_S1` を生成できないため `L41` が経済的に意味を成さない | ノード `Y_S1` を追加（§2.1）。エッジ `L60`（`COMPUTE_DEM → Y_S1`、行動・`+`・遅れ 0・saturating）・`L61`（`CAP_S1 → Y_S1`、制度・`+`・遅れ 0・saturating）を追加（§3.2）。ブロック C の添字集合を `SF = {S1, S2, S3}` と明記（§2.2・§3.4）。`PRICE_S1` は外生でありノードを置かない |
| `A2` | #165 §7 | 増幅ループ `R1` の閉路が部門を跨いで閉じており（`CASH_s`（`SP`）→ `CAPEX_EXEC`（`S1`））経済的に成立しない | `R1` を `R1a`（`S1` 内部）と `R1b`（`SP` の受注・内部資金）へ分割（§4）。`L41` の原因を `CASH_S1` に限定し、`SP` の内部資金経路を `L62`（`CASH_s → INVEST_s`）として新設（§3.5）。`A2` が挙げた意図 (i)・(ii) を**両方**採る |
| `B3` | #166 §10.2 | `CAPEX_EXEC` が発注ベースか引渡ベースか不明で、`L09`–`L11` が `CAPEX_EXEC` を制御変数でなくする | `CAPEX_EXEC` を「発注・引渡・支払が同一期に生じる支出額」と確定（§3.2 の根拠・注意）。`L11` の飽和が作用するのは一般需要・モデル外需要の未充足分のみ |
| `B5` | #166 §10.2 | 借換条件の悪化が現金流出を生む経路（`ROLLOVER → CASH_s`）がグラフに無い | `L63`（`ROLLOVER → CASH_s`、会計・`+`・遅れ 0）を追加（§3.5） |
| `B6` | #166 §10.2 | `L22`（`SALES_s → PROFIT_s`）が型 `行動` であるため `PROFIT_s` が `control` になるが、会計上は残差であり行動方程式で独立に生成できない | `L22` の型を `行動` → `会計` へ変更（§3.4）。役割の `diagnostic` への変更は #165 §4.4 |
| `B7` | #166 §10.2（確定記録） | `L41` の遅れ `0–1` のうち `0` を採ると期内処理順序が閉じない | `L41` の遅れを `1` に確定（§3.5）。`L62` も同じ理由で `1` とする |

### 10.2 本版で新設したエッジと変更の一覧

| ID | 変更 | 型 | 符号 | 遅れ | 由来 |
|---|---|---|---|---|---|
| `L60` | 新設 `COMPUTE_DEM → Y_S1` | 行動 | `+` | 0 | `A1` |
| `L61` | 新設 `CAP_S1 → Y_S1` | 制度 | `+` | 0 | `A1` |
| `L62` | 新設 `CASH_s → INVEST_s`（`s ∈ SP`） | 行動 | `+` | 1 | `A2` |
| `L63` | 新設 `ROLLOVER → CASH_s`（`s ∈ SF`） | 会計 | `+` | 0 | `B5` |
| `L64` | 新設 `COST_CAPITAL_s → INVEST_s`（`s ∈ SP`） | 行動 | `−` | 1 | 本版（下記） |
| `L22` | 型を `行動` → `会計` へ変更 | 会計 | `+` | 0 | `B6` |
| `L41` | 原因を `CASH_S1` に限定、遅れを `1` に確定 | 行動 | `+` | 1 | `A2`・`B7` |

**`L64` を新設した根拠**: `1.0.0` は `COST_CAPITAL_s` を `s ∈ SF` の潜在ノードとして定義しながら（§2.3・`L36`–`L38`）、その出力エッジを `L39`（`CAPEX_PLAN`、`S1` のノード）のみとしていた。したがって `COST_CAPITAL_S2`・`COST_CAPITAL_S3` は入力エッジのみを持ち、どこにも作用しない**行き止まりのノード**だった。`SP` の投資が資本コストに反応しないという主張はブロック D の趣旨（信用チャネル）と整合せず、`SP` の債務・スプレッドを持つ設計（`DEBT_s`・`COVERAGE_s` が `s ∈ SF`）とも整合しない。本版で `L64` を追加して解消する。

**契約**: `1.1.0` で解決した項目について、#165 §7 の `A1`・`A2`、#166 §10.2 の `B3`・`B5`・`B6` を**解決済みとして扱ってよい**。#166 §10.2 の `B1`・`B2`（`CAP_s` の単位・`ship_s` の変数化）は #165 の改訂事項であり、本書の対象外である。
...
| `Y_S5 → COMPUTE_DEM`（家計所得から `S1` 需要への還流） | 本版で追加しない。[動学方程式](capex_credit_cycle_equations.md) §17 の差し戻し `E1` |
...
| version | 日付 | 変更 |
|---|---|---|
| `capex-credit-cycle-graph/1.1.0` | 2026-07-30 | #169 の要求に基づく差し戻し解決（#165 `A1`・`A2`、#166 `B3`・`B5`・`B6`・`B7`）。ノード `Y_S1` を追加（37 個）。エッジ `L60`–`L64` を追加（64 本）。収益ブロックの添字集合を `SF = {S1, S2, S3}` へ明記。`L22` の型を `行動` → `会計` へ変更。`L41` の原因を `CASH_S1` に限定し遅れを `1` に確定。増幅ループ `R1` を `R1a`・`R1b` へ分割（5 本）。`B2` の作用先を 4 ループへ更新。`credit-off` で固定するエッジに `L64` を追加。差し戻し解決記録（§10）を追加 |
