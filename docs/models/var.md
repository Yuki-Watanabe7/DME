...
本実装（`VARModel`）は Phase 3 の目的に合わせて **係数行列を手入力する簡易 VAR(1)**として提供します。これにより、Ramsey・RBC などの理論モデルとは異なるアプローチで、複数変数の線形動学シミュレーションと簡易 IRF を扱えます。

Phase 5 以降では実データからの係数推定（OLS/Bayesian VAR）を対象とする予定です。詳細は [非対象項目](#非対象項目) を参照してください。
...
本実装は Phase 3 の簡易 VAR です。以下の項目は対象外です。
...
- **実データからの係数推定は対象外**
- OLS による推定、ベイズ VAR（BVAR）などは **Phase 5 以降** で対応予定
