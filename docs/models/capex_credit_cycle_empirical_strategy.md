...
| **ステータス** | 観測方程式・識別戦略・検証契約を確定。Julia の型・API・失敗契約・artifact は [実証統合設計](../architecture/capex_credit_cycle_empirical_integration.md)（#240）で確定済み。実装は catalog・raw observation・measurement・dataset・較正（#241–#244）、識別診断（#245）、ブロック別限定推定と parameter artifact（#246）、履歴再生候補選定（#247）、履歴再生実行（#248）、dimension別検証（#249）、robustness/sensitivity（#250）、統合デモ・artifact・利用文書（#251）まで完了。Phase 3（実データ接続・較正・履歴再生・検証・統合デモ）は完了 |
...
本節は #171 の横断整合レビュー（[統合設計](../architecture/capex_credit_cycle_integration.md) §2）で検出された不一致のうち本書が担当する 6 件（`X-14`・`X-20`・`X-22`–`X-25`）の解決を記録する。**本節は本書の正本であり、本文の該当箇所と矛盾する場合は本節が優先する。**
...
**理由**（§7.2 に記載済み。本節で引き渡し変更の事実として明示する）: `st_maturity_s`（部門別の平均満期）は企業開示に依拠する `A` 分類の量であり、[ADR 0012](../adr/0012-capex-credit-cycle-empirical-contract.md) 決定 6（企業開示を較正入力から除外する）に該当する。公開系列に部門別の平均満期は存在しない。したがって既定値を置き、走査範囲を定める `SENS` として扱う。#166 §11 の引き渡し要求はこの決定により**充足されない**。
...
| 遅延バッファの列挙と宣言の一致（#169 §21.2） | 状態次元は **65** のまま変わらない。§10.5 のヤコビアン評価回数の見積り（`65 + 1 = 66` 回 / 期）は変更を要しない |
