...
Keen モデルでは状態変数・動学シミュレーション・ショック分析・LLM 接続が利用可能である（[keen.md](keen.md)）。Minsky の資金調達区分は、企業の**期待キャッシュフローが利払いと元本返済をどこまで賄えるか**に基づく分類である。
...
既定値は**理論的アンカーではなく作業仮定**であり、実データによる推定・校正は将来対応予定である（§7）。
...
実データ系列でノイズが過大と判明した場合の hysteresis 導入余地は ADR に将来オプションとして記録する。
...
| 校正は将来対応予定 | 実データを用いた `amortization_rate` 推定・境界閾値の校正は将来対応予定。本書の既定値は作業仮定 |
...
- 実データによる `amortization_rate` 推定・閾値校正（将来対応予定）
