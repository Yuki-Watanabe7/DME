...
恣意的な単一スコアへの早期集約は解釈可能性を失うため、複数指標の併記を基本とし、
単一複合スコアは提供しない（§6）。
...
| 単一複合スコアなし | 複合スコアは提供しない（§6）。将来追加する場合も予測値として扱わない |
