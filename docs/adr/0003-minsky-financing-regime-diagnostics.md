...
Roadmap #99 の Phase 2 では Minsky の資金調達区分（Hedge / Speculative / Ponzi）判定を予定する。
...
- **hysteresis の初版導入**: 上記のとおり経路依存・再現性コストに見合わない。
  実データ系列（Phase 3）でノイズが過大と判明した場合の**将来オプション**として残す。
...
- 実データによる `amortization_rate` 推定・閾値校正・hysteresis の要否判断は Phase 3 に持ち越す。
